...
boolean Adafruit_VL53L0X::begin(uint8_t i2c_addr, boolean debug, TwoWire *i2c,
    ...
    if (Status == VL53L0X_ERROR_NONE) {
        ...
        Status =
            VL53L0X_PerformRefCalibration(pMyDevice, &VhvSettings,
                                          &PhaseCal);  // Device Initialization
    }

    if (Status == VL53L0X_ERROR_NONE) {
        // no need to do this when we use
...
VL53L0X_Error Adafruit_VL53L0X::setInterruptThresholds(
    ...
    if (Status == VL53L0X_ERROR_NONE) {
        ...
        // ST API Comments "no dependency on DeviceMode for Ewok " so device
        // mode not used but API requires something so pass in
...
VL53L0X_Error Adafruit_VL53L0X::getGpioConfig(
    ...
    if (Status == VL53L0X_ERROR_NONE) {
        ...
        Status = VL53L0X_GetGpioConfig(pMyDevice, 0, pDeviceMode,
                                       pFunctionality, pPolarity);
    }
...
boolean Adafruit_VL53L0X::setVcselPulsePeriod(
    ...
    Status = VL53L0X_SetVcselPulsePeriod(pMyDevice, VcselPeriodType,
                                         VCSELPulsePeriod);
    return (Status == VL53L0X_ERROR_NONE);
}
